...
def read_csv(path: Path, *, bbl: bool = False) -> pd.DataFrame:
...
def main() -> None:
    ...
    latest = registrations[registrations["is_latest_registration"].astype(str).str.lower().eq("true")].copy()
    print(
        f"Loaded {len(source_records):,} source rows, {len(rgb):,} source BBLs, "
        f"{len(pluto):,} PLUTO matches, {len(latest):,} latest registrations, "
        f"and {len(contacts):,} contacts"
    )
    ...
    violation_summary, recent, coord_by_bbl = b.normalize_violations(violations_raw, unit_lookup)
    ...
    housing = b.build_housing(rgb, pluto, registrations, contacts, violation_summary, coord_by_bbl)
    ...
    qa = {
        ...
        "requests": {"violations": violation_requests},
